...
普通用户的验证路径保持单一：日常评估统一走 `comet eval`。`/comet-any` 可以在内部登记 eval
evidence，但不要把内部登记步骤包装成面向普通用户的替代命令。对普通用户，解释时优先使用“验证”，不要把
`Publish readiness:` 当成第一层概念。
...
只有当前 draft hash 的 eval evidence 可以推进状态。旧 hash 证据可以保留在磁盘用于审计，但不能让当前
draft 进入 ready。
...
- `planHash`、`preferenceHash` 与 `reference/resolved-skills.json` 真实 Skill 证据，包括项目级偏好模式、required
  Skill、`sourceSummaries` 与“整理后的工作方式”摘要。
- 推荐调用顺序与 `preferenceIndex`。
- 偏离偏好顺序的项和原因。
- `.comet/skill-preferences.yaml` 是否在 Factory 初始化后发生漂移；`advisory` 模式给出 warning，
  `strict` 模式阻塞。
- 稳定组合 Skill Bundle 的 required capability set（必需能力集合）
  `skills/scripts/rules/hooks/references` 是否声明完整，且 `scripts/rules/hooks` 是否继续作为
...
readiness blockers 会阻止 publish。只要存在当前 draft hash 的 eval evidence 缺失、人工 approval
缺失、required capability gap 或 executable disclosure 未确认，就必须停在评审阶段，不能继续发布。
